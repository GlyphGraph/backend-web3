...
contract GlyphGraph is ERC721URIStorage, IGlyphGraph {
    ...
    mapping(address => User) public __users;
    event emitUser(
        string _id,
        string email,
        string name,
    ...
    function createUser(
        ...
        require(bytes(__users[msg.sender]._id).length == 0, "User already exists");
        ...
        console.log("object created", _user._id);
        __users[msg.sender] = _user;
        console.log(msg.sender);
        console.log("mapping set", __users[msg.sender]._id);
        emit emitUser(_randomId, _email, _name, _password, block.timestamp);
        // __users[msg.sender]._id = _randomId;
        // __users[msg.sender].name = _name;
        // __users[msg.sender].email = _email;
        // __users[msg.sender].password = _password;
        // __users[msg.sender].timestamp = block.timestamp;
    }

    function getUser() public view returns (User memory) {
        console.log(msg.sender, "hello", __users[msg.sender]._id);
